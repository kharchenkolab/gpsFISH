                             ...
                             ...)
{
  ...
  if (!useparallel) {
    ...
    #for test run#
    ##############
    # getfitness <- function(P, full_count_table, cell_cluster_conversion, nCV, rate, cluster_size_max, cluster_size_min, two_step_sampling_type, metric, return_type, method, simulation_parameter, simulation_model, weight_penalty, relative_prop, sample_new_levels, use_average_cluster_profiles) {         ###modified
  ...
  if (useparallel) {
    ...
    getfitness <- function(P, cluster) {
      mclapply(1:dim(P)[1], submit_fcn, pop.matrix = P, mc.cores = cluster)
    ...
    # fcn <- OF
    # submit_fcn <- function(index, pop.matrix,
    #                        full_count_table, cell_cluster_conversion, nCV, rate, cluster_size_max, cluster_size_min, two_step_sampling_type, metric, return_type, method, simulation_parameter, simulation_model, weight_penalty, relative_prop, sample_new_levels, use_average_cluster_profiles){
    #   fcn(string = pop.matrix[index,], full_count_table = full_count_table, cell_cluster_conversion = cell_cluster_conversion, nCV = nCV, rate = rate, cluster_size_max = cluster_size_max, cluster_size_min = cluster_size_min, two_step_sampling_type = two_step_sampling_type, metric = metric, return_type = return_type, method = method, simulation_parameter = simulation_parameter, simulation_model = simulation_model, weight_penalty = weight_penalty, relative_prop = relative_prop, sample_new_levels = sample_new_levels, use_average_cluster_profiles = use_average_cluster_profiles)
    # }
    # getfitness <- function(P, full_count_table, cell_cluster_conversion, nCV, rate, cluster_size_max, cluster_size_min, two_step_sampling_type, metric, return_type, method, simulation_parameter, simulation_model, weight_penalty, relative_prop, sample_new_levels, use_average_cluster_profiles, cluster) {
    #   mclapply(1:dim(P)[1], submit_fcn,
    #            pop.matrix = P,
    #            full_count_table = full_count_table, cell_cluster_conversion = cell_cluster_conversion, nCV = nCV, rate = rate, cluster_size_max = cluster_size_max, cluster_size_min = cluster_size_min, two_step_sampling_type = two_step_sampling_type, metric = metric, return_type = return_type, method = method, simulation_parameter = simulation_parameter, simulation_model = simulation_model, weight_penalty = weight_penalty, relative_prop = relative_prop, sample_new_levels = sample_new_levels, use_average_cluster_profiles = use_average_cluster_profiles,
    #            mc.cores = cluster, mc.preschedule = FALSE, affinity.list = rep(1:cluster,length.out=dim(P)[1]))
    # }
  }


  print("calculation preparation finished successfully")
  saveRDS(pop, "initial_pop.rds")

  #####################################
  #Actual calculation of fitness value#
  #####################################
  ####for single-thread case or multi-thread without sharedmemory, calculate fitness on the initial population###
  if (useparallel){
    #multi-thread & no shared memory
    ##############
    #for real run#
    ...
    # pop.fitness = getfitness(pop, full_count_table = full_count_table, cell_cluster_conversion = sc_cluster, nCV = params$nCV, rate = params$rate, cluster_size_max = cluster_size_max, cluster_size_min = cluster_size_min, two_step_sampling_type=params$sampling_type, metric=params$metric, return_type="fitness", method = method, simulation_parameter=simulation_parameter, simulation_model=simulation_model, weight_penalty=weight_penalty, relative_prop=relative_prop, sample_new_levels=sample_new_levels, use_average_cluster_profiles = use_average_cluster_profiles, cluster = cluster)    ###modified
  ...
  if (!useparallel){
    ...
    #for real run#
    ##############
    pop.fitness = getfitness(pop)                  #for single thread, the getfitness function is a modified version of OF
    ##############
    #for test run#
    ##############
    #pop.fitness = getfitness(pop, full_count_table = full_count_table, cell_cluster_conversion = sc_cluster, nCV = params$nCV, rate = params$rate, cluster_size_max = cluster_size_max, cluster_size_min = cluster_size_min, two_step_sampling_type=params$sampling_type, metric=params$metric, return_type="fitness", method = method, simulation_parameter=simulation_parameter, simulation_model=simulation_model, weight_penalty=weight_penalty, relative_prop=relative_prop, sample_new_levels=sample_new_levels, use_average_cluster_profiles = use_average_cluster_profiles)    ###modified
  ...
  print("Initial population fitness calculation finished successfully")
  ...
  if (verbose > 0) {
    cat("Initial population best OF value = ", old$obj[1],
  ...
  for (gen in 1:ngen) {
    ...
    pickfun <- function(v) sample(v, 1, prob = rank(-fitness.old[v]))       #solutions/chromosomes with low fitness value -> higher probability of being picked
    ...
    for (i in 1:popsize) {                                                  #generate offspring through crossover
      ...
      if (is.null(gene.weight)){
        ...
        duplicated.loc <- duplicated(offspring[i, ])         #there may be duplications
        if (sum(duplicated.loc)>0){
          crossover.candidate <- setdiff(combo, offspring[i, ])
        ...
        if (sum(duplicated.loc)>0){                          #if there are duplications
          replacement.pool <- setdiff(combo, offspring[i, ])      #we replace those duplications with genes in the two parents but not in the current offspring
    ...
    #The whole idea up to this point is the for each chromosome in the new population,
    #we first randomly select tourneysize chromosomes from the previous generation as candidates for Parents1.
    #Then we randomly another tourneysize chromosomes from the previous generation as candidates for Parents2.
    #Among the tourneysize candidates, we randomly select one with probability proportional to its fitness value as the final parent.
    #Then we crossover the two parents.
    #Larger tourneysize will lead to more candidate for final parent and will have a higher probability of covering chromosome with better fitness
    #But this may also reduce diversity

    print(paste("iteration", gen, "crossover no problem"))
    ...
    for (i in 1:popsize) {                                                  #for each solution
      if (nchosen[i] > 0) {                                                 #if we have at least one gene to mutate
        ...
        # if (length(unique(offspring[i, ]))<50){
        #   saveRDS(list(nchosen=nchosen[i], candidates=candidates, toadd=toadd, chosen.loc = chosen[i, ], offspring.panel = offspring[i, ]), "problem_in_mutation.rds")
        # }
      }
      ...
      if (!is.null(gene2include.id)){
        ...
        already.in = intersect(offspring[i,], gene2include.id)          #must include genes that are already in the current solution
        if (length(already.in)<length(gene2include.id)){                #if some/all of the must include genes are not in the solution
          if (length(already.in)>0){                                    #if a subset of must include genes are already in the solution
            ...
            offspring[i, pos2change] = setdiff(gene2include.id, already.in)
          }
    ...
    print(paste("iteration", gen, "mutation no problem"))
    # saveRDS(offspring, paste0("offspring",gen,".rds"))
    ...
    if (useparallel){
      ...
      #for test run#
      ##############
      #pop.fitness = getfitness(offspring, full_count_table = full_count_table, cell_cluster_conversion = sc_cluster, nCV = params$nCV, rate = params$rate, cluster_size_max = cluster_size_max, cluster_size_min = cluster_size_min, two_step_sampling_type=params$sampling_type, metric=params$metric, return_type="fitness", method = method, simulation_parameter=simulation_parameter, simulation_model=params$simulation_model, weight_penalty=weight_penalty, relative_prop=relative_prop, sample_new_levels=sample_new_levels, use_average_cluster_profiles = use_average_cluster_profiles, cluster = cluster)    ###modified
    ...
    print(paste("iteration", gen, "fitness calculation no problem"))
    ...
    if (keepbest == 0) {                                                  #update population when keepbest==0
      ...
      old.keep <- rank(fitness.old, ties.method = "first") <= keepbest                #we keep the top keepbest solutions in the old population
      new.keep <- rank(fitness.new, ties.method = "first") <= popsize - keepbest      #for the rest of the solutions, we replace them with the top (popsize-keepbest) solutions from the offspring population
      #we change ties.method to "first" because this is how we select the best solution in each round (the first one with minimum fitness value is the best solution).
      #In order to make sure the best solution is included in the population when keepbest>0 and popsize-keepbest>0 and there are ties, we use "first".
      #for example, when popsize-keepbest=2 but we have 4 solutions tie with best fitness value, a random selection may not include the first one (i.e., the best solution) in the population
      #However, if we use "first", we will make sure that as long as popsize-keepbest>0, the best solution (best.pos) will always be included
      ...
      #Now pop is a combination of top solutions in the previous round and top solutions in the current round
      #And fitness.old contains the fitness value of this mixture of top solutions
    }

    ###save the result for the current generation###
    #Of note:
    #When keepbest != 0, the best solution in each round (except for the first round) is no longer gauranteed to be the best solution in each offspring population
    #Because the best solution is selected from a mixture of solutions in the previous population and offspring population
    #If the selected best solution is from the previous population, then all the rest of the information (best_confusionMatrix, best_AUC_by_class, best_feature_imp, etc)
    #will not match the current best solution because these information are just the best given solutions in offspring population. Therefore, we need to modify them backwards
    #If keepbest==0, then all the information will agree with the best solution in each generation
    ...
    print(paste("iteration", gen, "result update no problem"))
